// /api/grade.js - 영작 채점 (Gemini 2.5 Flash-Lite) - 엄격 모드
...
반드시 JSON 형식으로만 응답:
{
  ...
  "total": 총점,
  "corrections": [
    "❌ 'I am charge of' → ✅ 'I am in charge of' · 청크에서 'in' 빠짐. 'be in charge of'가 정확한 표현",
    "❌ 'last year' → ✅ '~' · 시제 이유"
  ],
  "good": "구체적으로 잘한 점 (한국어 1문장, 진짜 잘했을 때만)",
  "tip": "약점 1개 콕 짚어서 다음 단계 (한국어 1-2문장)",
  "cheer": "솔직한 응원 (과장된 칭찬 X)"
}

corrections는 명백한 실수를 모두 잡아야 합니다. 못 본 척 넘어가지 마세요.`;

  const userPrompt = `[상황]\n${situation || ''}\n\n[태호님 영작]\n${text}\n\n엄격하게 채점해주세요.\n특히:\n1. 청크(${(chunks || []).join(', ')})를 정확히 사용했는지\n2. 한국인 흔한 실수 (관사, 전치사, 시제) 다 짚었는지\n3. 90점 이상은 진짜 원어민 수준일 때만`;
